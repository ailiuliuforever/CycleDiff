...
def main(args):
    ...
    swanlab.init(
        ...
        description=f"VAE training for RSI2Map task using {experiment_name} configuration",
        config={
            'model_class': model_cfg.get('class_name', 'AutoencoderKL'),
            'embed_dim': model_cfg.get('embed_dim', 3),
            'batch_size': data_cfg.get('batch_size', 16),
            'learning_rate': train_cfg.get('lr', 1e-4),
            'min_lr': train_cfg.get('min_lr', 1e-6),
            'train_num_steps': train_cfg.get('train_num_steps', 100000),
            'image_size': model_cfg.get('ddconfig', {}).get('resolution', [256, 256]),
    ...
    trainer = Trainer(
        ...
        amp=train_cfg['amp'], fp16=train_cfg['fp16'], log_freq= train_cfg['log_freq'], cfg=cfg, resume_milestone=train_cfg['resume_milestone']
    )
    trainer.train()
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            self.save(milestone)
                            if isinstance(self.model, nn.parallel.DistributedDataParallel):
                                all_images = self.model.module.validate_img(img)
                            elif isinstance(self.model, nn.Module):
                                all_images = self.model.validate_img(img)
                        ...
                        try:
                            swanlab.log({
                                'reconstructed_images': swanlab.Image(sample_path, caption=f"Step {self.step} - Milestone {milestone}")
                            })
