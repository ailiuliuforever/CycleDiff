...
class Trainer(object):
    def __init__(
        ...
        self.results_folder = Path(results_folder)
        if self.accelerator.is_main_process:
            # 将 CfgNode 转换为普通字典，确保所有配置参数都被记录
            ...
            swanlab.init(
                ...
                experiment_name=f"ldm_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                config=config_dict,
                description="Latent Diffusion Model Training for RSI to Map Translation"
            )
    ...
    def load(self, milestone):
        ...
        if 'scale_factor' in data['model']:
            if self.cfg.model.scale_factor != None:
                self.model.scale_factor = torch.tensor(self.cfg.model.scale_factor, device=device)
            else:
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        swanlab.log({
                            "generated_samples": swanlab.Image(all_images, caption=f"Sample at step {self.step}")
                        }, step=self.step)
                        self.model.train()
                accelerator.wait_for_everyone()
